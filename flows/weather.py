...
@flow()
def get_weather_data(logger):
    temperature = get_temperature(logger, LAT, LONG)
    air_quality = get_air_quality(logger, LAT, LONG)
    elevation = get_elevation(logger, LAT, LONG)
    logger.info(f"{temperature}\n{air_quality}\n{elevation}")
    return temperature, air_quality, elevation


@flow()
def weather_superflow():
    logger = get_run_logger()
    temperature, air_quality, elevation = get_weather_data(logger)
